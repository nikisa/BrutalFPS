...
public class AIZombieState_Alerted1 : AIZombieState {
    ...
    public override void OnEnterState() {
        //Inizializza il timer e configura la SM
    ...
    public override AIStateType OnUpdate() {
        ...
        // Se disponibile passo allo stato di Patrol
        if (_timer <= 0.0f) {
        ...
        // La minaccia visiva è un Player? Prende la precedenza rispetto le minacce audio
        if (_zombieStateMachine.VisualThreat.type == AITargetType.Visual_Player) {
            _zombieStateMachine.SetTarget(_zombieStateMachine.VisualThreat);

            if (_screamChance>0.0f && Time.time > _nextScream) {
                if (_zombieStateMachine.Scream()) {
                    _screamChance = float.MinValue; //In modo da non triggerare ancora screamChance
        ...
        if (!_zombieStateMachine.useRootRotation) {
            _zombieStateMachine.transform.Rotate(new Vector3(0.0f, _slerpSpeed * _zombieStateMachine.seeking * Time.deltaTime));
        }
